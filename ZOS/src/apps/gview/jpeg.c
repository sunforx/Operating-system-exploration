...
typedef struct
{
    ...
    int scan_ac[3];
    int scan_dc[3];
    int scan_h[3];  // ç—?å≥ëfìIêîó 
    int scan_v[3];  // ç—?å≥ëfìIêîó 
    int scan_qt[3]; // ó âªï\çıà¯
    
    // DRI
    int interval;

    int mcu_buf[32*32*4]; // ?ôt
    int *mcu_yuv[4];
    int mcu_preDC[3];
	...
	int base_img[64][64]; // äÓíÍâÊëú ( [â°é¸îgêîuÉŒ][?é¸îgêîvÉŒ][â°à ëä(M/8)][?à ëä(N/8)]
...
//	if (jpeg->width == 0)
//		return 8;
	/* decode0èäà»õÄâÔç›êMëßîVç@îÌ?ópÅCéß?ïsê• */
...
unsigned short get_bits(JPEG *jpeg, int bit)
{
	...
	while (remain <= 16) {
		...
		c = *jpeg->fp++;
		if (c == 0xff) { // ?óπñhé~????ÅCõÄîÌ?à⁄ìûFF - > FF 00
			if (jpeg->fp >= jpeg->fp1) {
				ret = 0;
				goto fin;
			}
	...
	while (size > 0) {
		...
		if (j > jpeg->n_dqt)
 		...
 		if (c & 0xf8) {
			// 16 bit DQT
			for (i = 0; i < 64; i++) {
				jpeg->dqt[j][i] = jpeg->fp[0]; /* â∫îºïîï™?úP */
				jpeg->fp += 2;
			}
			size += -64 * 2;
		} else {
...
int jpeg_dht(JPEG *jpeg)
{
	...
	while (len > 0) {
		...
		tc = (val >> 4) & 0x0f; // ï\?(DC/AC?åè??äÌ)
		th =  val       & 0x0f; // ï\?(õÄóLëΩè≠)
...
int jpeg_decode_mcu(JPEG *jpeg, UCHAR *zigzag_table)
{
	...
	// mcu_width x mcu_height?ëÂëÂè¨ìI?
